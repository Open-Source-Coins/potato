...
uint256 hashGenesisBlock("0x00000f9c6f1706cd7ef6f14eb68a36a2debd1f4f917dd75fac489bc2a0e8add7");
...
int64 static GetBlockValue(int nBits, int nHeight, int64 nFees)
{
    ...
    if(nHeight == 1)
    {
       return 187500000 * COIN; // Premine
    }
    else if(nHeight == 2)
    {
       return 78125000 * COIN;
    }
    else if(nHeight == 3)
    {
       return 31250000 * COIN;
    }
    else if(nHeight == 4)
    {
       return 15625000 * COIN;
    }
    else if(nHeight > 4)
    {
       nSubsidy = 500 * COIN; // 500 coins for each block
    }

    if(nHeight * 0.0004 > 500) // We prevent block value from going to negative
    {
       return nFees; // POS stage starts and we only pay out fees to miners
    }
    else
    {
       return nSubsidy + nFees - nHeight * 0.0004 * COIN; // Constant decay formula to calculate block value
    }

}

static const int64 nTargetTimespan = 1 * 30 * 1; // potatocoin: every 1 blocks
static const int64 nTargetSpacing = 1 * 30; // potatocoin: 30 seconds
...
unsigned int static DarkGravityWave(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, potatocoin - DarkGravity, written by Evan Duffield - evan@potatocoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
unsigned int static DarkGravityWave2(const CBlockIndex* pindexLast, const CBlockHeader *pblock) {
    /* current difficulty formula, potatocoin - DarkGravity v2, written by Evan Duffield - evan@potatocoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlockHeader *BlockCreating = pblock;
    BlockCreating = BlockCreating;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Ago-2014 In search of Obama's Iraq strategy";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1407764172;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 976601;

        if (fTestNet)
        {
            block.nTime    = 1407596953;
            block.nNonce   = 0;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe8e655479662bd091d6f414ceb4352932c051a327acf21fdebd7905e5cb9c8a9"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xa3, 0xd5, 0xc2, 0xf9 }; // potatocoin: increase each by adding 2 to bitcoin's value.
...
CBlockTemplate* CreateNewBlock(const CScript& scriptPubKeyIn)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            // transactions:
            if (!fSortedByFee &&
                ((nBlockSize + nTxSize >= nBlockPrioritySize) || (dPriority < COIN * 2280 / 250)))
            {
                fSortedByFee = true;
